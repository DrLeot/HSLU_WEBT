...
function upsertMenuData($connection, $values){
    ...
    foreach($values as $key=> $value){
        if(!isset($value) or empty($value)){
            printWarning($key." seems not to be set. Data to DB will not be imported or updated.");
            $ret = true;
        }
    }
    if($ret){
    ...
    if($number_of_existing > 0){
        ...
        $sql_menu_update->bind_param("ssis", $values["category"], $values["ingredients"], $values["servesfor"], $values["menuname"]);
        ...
        $sql_menu_insert->bind_param("sssi", $values["menuname"], $values["category"], $values["ingredients"], $values["servesfor"]);
